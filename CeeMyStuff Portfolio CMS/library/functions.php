...
function destroyHTML($str, $tags='<a><p><b><i><strong><em><u><del><span><h1><h2><h3><h4><h5><h6>', $invert=false) {
...
function insertFilenameTag($filename, $tag, $forUpload=false, $underscore=true) {
    $fileTypeOG = pathinfo($filename,PATHINFO_EXTENSION);
...
function echoIfTesting($input,$tag=null,$echoNow=true,$onlyID=false){
    global $testing;
    if ($onlyID && $onlyID!=$_SESSION['UserID']) {
        return;
    }
...
if (isset($_POST['send_password_reset'])) {
    ...
    if ($emailValid) {
        ...
        $stmt = $conn->prepare($updtQry);
        $time = time(); 
        $time = addToUnixTime($time,15,"minutes");
        ...
        if ($failed === false) {
            ...
            This activation key expires on '.timestampToDate($time);
                if (mail($email, 'CeeMyStuff Password Reset', $body, $emailHeaders)) {
                    ...
                    You have 15 minutes before your activation key for the new passwords expires (".timestampToDate($time).").</p>";
                    $msg=fdbkMsg(1,$msg,null,true);
                    return;
                }
                // //FOR TESTING 
...
function getSectItems($sect, $pageNum=1, $pgAfter=null, $tag=null, $orderBy=null, $orderDir=null) {
    ...
    switch ($setShowText) {
        ...
        case 3: // custom truncation
            if (str_contains($text, '<!--truncate-->')) {
                $text=truncateTxt($text, "Custom", null);
            }
...
if (isset($_POST['change_user_icon'])) {
    ...
    if ($iconUpld) {
        ...
        $iconPath = uploadImage($dir, $_FILES['icon_upload'], 120, 120, false, false, false, $set['max_upld_storage']);
    ...
    if ($iconPath || $rmvIcon) {
        ...
        if ($stmt->execute()) {
            ...
            if ($iconPath) {
                mkThumb($dir, $iconUpld, $iconPath, 50, 50, false, false, "50px");
            }
        } else {
...
function createItem($cPost) {
    global $db;
    $conn = new SQLite3($db);
    $qry = "INSERT INTO `Items` (`Posted_By_ID`,`Sect_ID`,`Title`,`Img_Path`,`Img_Alt_Text`,`File_Path`,`File_Link_Text`,
    `Embed_HTML`,`Img_Thumb_Path`,`Text`,`Publish_Timestamp`,`Tags`,`Hidden`,`Format`)
    VALUES (:userid,:sectid,:title,:img,:alttext,:file,:linktext,:embed,:imgthumb,:text,:ts,:tags,:hide,:format);";
    $stmt = $conn->prepare($qry);
    $stmt->bindValue(':userid',$cPost['n_user_id'], SQLITE3_INTEGER);
...
if (isset($_POST['create_section']) || isset($_POST['edit_section'])) {
    ...
    $cPost['text'] = closeHTML($cPost['b_text'] ?? $cPost['m_text']);
    //
    ...
    if (!$exec) {
        if ($_SESSION['Msg']) {
            ...
            $msg ="Changes to this section failed to save. Please try again.";
        }
        $msg=fdbkMsg(0,$msg);
    } else {
        $msg=fdbkMsg(1,"Section settings were saved at ".date('d F, Y h:i:s').'.');
    }
}
...
if (isset($_POST['create_item']) || isset($_POST['edit_item'])) {
    ...
    global $set;
    $_POST['n_user_id'] = $_SESSION['UserID'];
    ...
    if ($imgUpld) {
        ...
        if (!$imgPath) {
            $msg = fdbkMsg(0,'Image upload failed. Please try again.');
            return;
        } else {
            $newImgUpld=true;
        }
    ...
    if (($newImgUpld && $createThumb) || ($imgPath && $createThumb)) {
        if ($thumbUpld) {
            ...
            if (!$imgThumbPath) {
                $msg = fdbkMsg(0,'Thumbnail image upload failed. Please try again.');
                return;
            }
        } else {
            if ($imgUpld) {
...
if (isset($_POST['admin_media_upload'])) {
    ...
    if ($filesLen) {
        ...
        for ($i=0;$i<$filesLen;$i++) {
            if ($files[$i]['tmp_name']) {
                $filePath = uploadFile($dir, $files[$i], false, $set['max_upld_storage']);
                if ($filePath) {
                    $successLen++;
                    ...
                    $error=true;
                    if ($msg) {$msg.="<br/>";}
                    $msg .= "<div class='red'>There was an error uploading '".$files[$i]['name']."'. Please try again.</div>";
                }
            }
        }
        if ($successLen) {
        ...
        if ($msg) {$msg.="<br/>";}
        $msg .= "No valid files were found to upload.";
        $msg=fdbkMsg(0,$msg);
    }
    return false;
}
...
if ($_SESSION['Permissions']>=$set['full_permissions']) {
